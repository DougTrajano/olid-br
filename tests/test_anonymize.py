...
TESTS = [
    ...
    },
    {
        ...
        "without_names": "@doug_trajano teste normal https://www.google.com.br",
    ...
    },
    {
        ...
        "without_names": "Números foram divulgados nesta segunda, um dia após ação na Praia do Porto de Santo USER USER USERUSER USERUSER USERentulho",
        "without_all": "Números foram divulgados nesta segunda, um dia após ação na Praia do Porto de Santo USER URL HASHTAG HASHTAG HASHTAG"
    },
    {
...
@pytest.mark.parametrize("text, output", [(i["input"], i["without_names"]) for i in TESTS])
def test_remove_names(text: str, output: str):
    anonymizer = Anonymizer()
    assert anonymizer.remove_names(text) == output
    print(f"TEST OK for Anonymizer().remove_names - Input: {text} - Output: {output}")
